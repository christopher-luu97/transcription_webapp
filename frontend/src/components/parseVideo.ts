...
export function parseVideoURL(input: string): VideoURL {
  ...
  if (WATCHV_REGEX.test(url.host)) {
    ...
    if (!params.has("v")) {
      throw new Error("Your URL must contain the watch?v part!");
    }

    id = params.get("v")!;
  } else if (url.host === "youtu.be") {
    if (url.pathname.length <= 1) {
      throw new Error("Your URL must contain the video ID at the end!");
    }

    id = url.pathname.substring(1);
  } else {
    throw new Error("Your URL must be from YouTube or from YouTube Music!");
  }
